...
function loadTable() {
            ...
            const tableData = localStorage.getItem(slideDeckId) ?? slidesArray;
                    ...
                    { title: "Index", field: "slideState",  responsive: 0,
                        cellClick: function gotoSlide(e, cell){
                            ...
                            if(cellValues.length === 3){
                                iframe.contentWindow.postMessage(JSON.stringify({ method: 'slide', args: [Number(cellValues[0]),Number(cellValues[1]),Number(cellValues[2])]}), 'https://slides.com');
                            }else{
                                iframe.contentWindow.postMessage(JSON.stringify({ method: 'slide', args: [Number(cellValues[0]),Number(cellValues[1])]}), 'https://slides.com');
                            }
                        },
                        sorter:"alphanum",
                    }, 
