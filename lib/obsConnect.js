...
window.addEventListener(`ws-details`, async function (event) {
  ...
  if(event.detail.hasOwnProperty('wssDetails') && connecting === false){
    connecting = true;
    wssDetails.IP = event.detail.wssDetails.IP
    wssDetails.PORT = event.detail.wssDetails.PORT
    wssDetails.PW = event.detail.wssDetails.PW
    const connectionStatus = await connectOBS(wssDetails);
    if(connectionStatus === "connected"){
      localStorage.setItem("wssDetails",wssDetails)
    }
  }
})
